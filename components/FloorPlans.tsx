      ...
      <div className="shell">
        <div className="floorHeading">
          <div>
            ...
            <h2 className="sectionTitle">상담북의 실제 평면으로 보는<br />6가지 주거 타입</h2>
          </div>
        ...
        <div className="planTabs" role="tablist" aria-label="주거 타입 선택">
          {plans.map((item) => (
            <button key={item.id} type="button" role="tab" aria-selected={selected === item.id} className={selected === item.id ? "active" : ""} onClick={() => setSelected(item.id)}>
              {item.id}
            </button>
          ))}
        </div>
        ...
        <div className="floorV60Panel">
          <div className="floorV60Image">
            <Image key={plan.image} src={plan.image} alt={`${plan.id} 타입 공식 평면도`} fill sizes="(max-width: 900px) 100vw, 68vw" />
          </div>
